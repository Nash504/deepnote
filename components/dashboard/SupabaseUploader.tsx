...
export default function SupabaseUploader({ type }: { type: string }) {
    ...
    instance.on("complete", async (result) => {
      ...
      for (const file of result.successful) {
        ...
        const { data, error } = await supabase.storage
          .from(type) // bucket name is the type (e.g. "question_paper" or "answer_paper")
        ...
        <div className="space-y-4">
          <div className="rounded-lg overflow-hidden border bg-background border-black">
            <Dashboard
              ...
              note={`Upload your ${type.replace(
                "-",
                " "
              )} here. Maximum 5 files.`}
            />
          </div>
